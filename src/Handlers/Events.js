...
/**
 * Event handler for group participant updates (joins, leaves, promotions, demotions)
 * @async
 * @param {Object} event - Event data from Baileys
...
const EventsHandler = async (event, client) => {
    try {
        ...
        if (event.action === 'add') {
            ...
            const {username} = await client.contact.getContact(user, client)
